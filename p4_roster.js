...
console.log(students.filter(obj => obj.score >= 80).map(obj => obj.name));
...
let result = students.slice().sort((a, b) => b.score - a.score);

for(let i = 0; i<10; i++) {
    console.log(i+1,". ", result[i].name," (", result[i].dept, ") ", result[i].score);
}
